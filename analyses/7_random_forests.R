...
data1 <- select(kelp_var, kelp.cover:toce)
set.seed(666)
...
rf_mtry_test <- function(mtry_num){
  ...
  pred_accuracy <- data.frame(mtry = mtry_num,
                              acc = round(mean(round(pred_test, -1) == train_set$kelp.cover)*100))
  return(pred_accuracy)
}
lapply(1:9, rf_mtry_test) # It looks like an mtry = 1 is best
...
table(round(pred_train, -1), train_set$kelp.cover)  
mean(round(pred_train, -1) == train_set$kelp.cover)*100 # 23% accuracy...

# Predicting on Validation set
pred_valid <- predict(kelp_rf, valid_set, type = "class")
table(round(pred_valid, -1), valid_set$kelp.cover)  
mean(round(pred_valid, -1) == valid_set$kelp.cover)*100 # 4% accuracy... sad face
...
mean(pred.forest.rk1$pred.for.obs.forest.rk == as.vector(y.test))*100 # 2% accuracy...
